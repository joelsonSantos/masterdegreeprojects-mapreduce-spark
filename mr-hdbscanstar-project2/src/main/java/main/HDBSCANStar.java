...
import main.Constraint.CONSTRAINT_TYPE;
...
public class HDBSCANStar implements Serializable {
	...
	public double[] calculateCoreDistances(double[][] dataSet, int k) {
		...
		for (int point = 0; point < dataSet.length; point++) {
			...
			for (int neighbor = 0; neighbor < dataSet.length; neighbor++) {
				double distance = new EuclideanDistance().computeDistance(dataSet[point], dataSet[neighbor]);
	 ...
	 */
	public UndirectedGraph constructMST(double[][] dataSet, double[] coreDistances, boolean selfEdges, int[] indices,
			int totalLength) {
		...
		while (numAttachedPoints < dataSet.length) {
			...
			for (int neighbor = 0; neighbor < dataSet.length; neighbor++) {
				...
				double distance = new EuclideanDistance().computeDistance(dataSet[currentPoint], dataSet[neighbor]);
	...
	public ArrayList<Cluster> computeHierarchyAndClusterTree(UndirectedGraph mst, int minClusterSize,
		...
		// Sets for the clusters and vertices that are affected by the edge(s) // being
		// removed:
		...
		while (currentEdgeIndex >= 0) {
			...
			// Remove all edges tied with the current edge weight, and store // relevant
			// clusters and vertices:
			...
			while (!affectedClusterLabels.isEmpty()) {
				...
				// Get all affected vertices that are members of the cluster // currently being
				// examined:
				...
				while (!examinedVertices.isEmpty()) {
					...
					// Explore this potential child cluster as long as there are // unexplored
					// points:
				...
				// Finish exploring and cluster the first child cluster if there // was a split
				// and it was not already clustered:
	...
	/**
	 * Propagates constraint satisfaction, stability, and lowest child death level
	 * from each child cluster to each parent cluster in the tree. This method must
	 * be called before calling findProminentClusters() or calculateOutlierScores().
	 * 
	 ...
	 *         otherwise
	 */
	public boolean propagateTree(ArrayList<Cluster> clusters) {
	...
	/**
	 * Produces a flat clustering result using constraint satisfaction and cluster
	 * stability, and returns an array of labels. propagateTree() must be called
	 ...
	 * @throws IOException           If any errors occur opening, reading, or
	 *                               writing to the files
	 * @throws NumberFormatException If illegal number values are found in the
	 *                               hierarchyFile
	 */
	public int[] findProminentClusters(ArrayList<Cluster> clusters, String hierarchyFile, String flatOutputFile,
	 ...
	 * @param edgeWeight    The edge weight at which to remove the points from their
	 *                      previous Cluster
